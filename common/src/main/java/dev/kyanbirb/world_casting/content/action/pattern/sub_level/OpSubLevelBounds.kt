...
class OpSubLevelBounds(val min: Boolean) : ConstMediaAction {
    override val argc = 1
    override fun execute(args: List<Iota>, env: CastingEnvironment): List<Iota> {
        val fragment = args.getFragment(0, argc)
        val subLevel = fragment.getSubLevel(env.world) as ServerSubLevel
        val aabb = subLevel.plot.boundingBox.toAABB()
        if(min) {
            return aabb.minPosition.asActionResult
        }
        return aabb.maxPosition.asActionResult
    }
}